...
public class EnableQueryPropertyTest {
    ...
    public void testAlterTableEnableQueryAndBlockQuery() throws Exception {
        ...
        // executing ALTER is done through StmtExecutor in utframe; reuse ShowCreateTable logic from existing tests
        ShowCreateTableStmt showStmt =
                new ShowCreateTableStmt(new TableName("test", "t_enable_query_2"),
                        ShowCreateTableStmt.CreateTableType.TABLE);
        com.starrocks.sql.analyzer.Analyzer.analyze(showStmt, ctx);
        ShowResultSet resultSet = ShowExecutor.execute(showStmt, ctx);
        String createSql = resultSet.getResultRows().get(0).get(1);
        Assertions.assertTrue(createSql.contains("\"enable_query\" = \"false\""),
        ...
        Exception ex = Assertions.assertThrows(Exception.class, () ->
                UtFrameUtils.parseStmtWithNewParser(querySql, ctx));
        // message format is checked in outer error handling; we only check core text here
        Assertions.assertTrue(ex.getMessage().contains("the table test.t_enable_query_2 enable_query is false"),
