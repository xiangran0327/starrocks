...
public class StatisticsEstimateUtils {
    ...
    /**
     * Gets selectivity from histogram MCV for equality predicate, or null if not available.
     */
    private static Double trySelectivityFromHistogramMcv(ColumnStatistic colStat, ConstantOperator constant) {
        if (colStat == null || constant == null || colStat.getHistogram() == null) {
            return null;
        }
        Map<String, Long> mcv = colStat.getHistogram().getMCV();
        if (mcv == null || mcv.isEmpty()) {
            return null;
        }
        String key = constant.toString();
        Long cnt = mcv.get(key);

        if (cnt == null) {
            return null;
        }
        double total = colStat.getHistogram().getTotalRows();
        return total <= 0 ? null : (cnt / total) * (1.0 - colStat.getNullsFraction());
    }
    ...
    private static Double trySelectivityFromFunctionalDependency(
        ...
        for (ColumnRefOperator col : correlatedColumns) {
            ...
            Double sel = trySelectivityFromHistogramMcv(colStat, equalityPredicates.get(col));
            if (sel == null) {
